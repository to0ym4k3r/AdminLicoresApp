...
/// Sheet de dictado por voz: graba la nota, detecta fecha/hora en el texto
/// y permite guardarla (con alarma única si se detectó una fecha futura).
struct VozNotaSheet: View {
    ...
    private var encabezado: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Dicta y la app detecta fechas y horas para crear la alarma")
    ...
    private var botonMicrofono: some View {
        VStack(spacing: 10) {
            ...
            Text(servicio.estaGrabando ? "Grabando… toca para detener" : "Toca el micrófono y dicta tu nota")
    ...
    private var botonGuardar: some View {
        let hayAlarma = detectada.map { $0 > Date() } ?? false
        return Button {
            viewModel.registrarNotaVoz(texto: texto, fechaDetectada: detectada)
            dismiss()
        } label: {
            HStack(spacing: 8) {
                Image(systemName: hayAlarma ? "bell.badge.fill" : "mic.fill")
                Text(hayAlarma ? "Guardar con alarma" : "Guardar nota")
